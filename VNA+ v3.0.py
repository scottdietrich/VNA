...
Version: 2.1 - Gate Safety Update (January 2026)
...
class VNAController:
    ...
    def setup_frequency_sweep(self, f_start, f_stop, num_points, ifbw, power):
        ...
        try:
            ...
            print(f"VNA confirms: IFBW={actual_ifbw.strip()}, Smooth={actual_smo.strip()}, Avg={actual_avg.strip()}")
...
class NHMFLMagnetController:
    ...
    def _get_status_raw(self):
        ...
        try:
            ...
            data = self.client.recv(self._get_byte_size(databytes))
            decoded = data.decode()
            parts = decoded.split(',')
            
            # Debug: print raw response occasionally
            if not hasattr(self, '_debug_count'):
                self._debug_count = 0
            self._debug_count += 1
            if self._debug_count <= 5 or self._debug_count % 20 == 0:
                print(f"  [SCM1 raw] {decoded}")
            
            return {
                'Field': float(parts[0]),
                'Setpoint': float(parts[1]),
                'SlewRate': float(parts[2]),
                'Ramp': parts[3] == '1' or parts[3].lower() == 'true',
                'Pause': parts[4] == '1' or parts[4].lower() == 'true' if len(parts) > 4 else False,
                'Units': int(parts[5]) if len(parts) > 5 else 0  # 0=Tesla, 1=kAmps
            }
        except Exception as e:
    ...
    def get_field(self):
        ...
        status = self._get_status_raw()
        if status:
            self.current_field = status['Field']
            return status['Field']
        return self.current_field
    ...
    def _send_command(self, cmd):
        ...
        Every command to LabVIEW requires reading the response to keep in sync.
        """
        try:
            self.client.send(self._send_data(cmd))
            # Must read response to stay in sync
    ...
    def set_field(self, target_field):
        ...
        try:
            # Set the setpoint (and read response)
            resp = self._send_command('s' + str(target_field))
            print(f"NHMFL SCM1: Set setpoint to {target_field:.4f} T, response: {resp}")
            
            time.sleep(0.1)
            
            # Start ramping (u1 = ramp to setpoint)
            resp = self._send_command('u1')
            print(f"NHMFL SCM1: Ramp command sent, response: {resp}")
    ...
    def set_rate(self, rate):
        ...
        try:
            resp = self._send_command('r' + str(rate))
            print(f"NHMFL SCM1: Slew rate set to {rate}, response: {resp}")
            return True
        except Exception as e:
    ...
    def ramp(self, start=True):
        ...
        try:
            resp = self._send_command('u' + str(int(start)))
            print(f"NHMFL SCM1: Ramp {'started' if start else 'stopped'}, response: {resp}")
            return True
        except Exception as e:
    ...
    def pause(self, paused=True):
        ...
        try:
            resp = self._send_command('p' + str(int(paused)))
            print(f"NHMFL SCM1: {'Paused' if paused else 'Resumed'}, response: {resp}")
            return True
        except Exception as e:
...
class KeithleyController:
    ...
    def _query_raw(self, command, timeout_ms=5000):
        """Raw query with explicit error handling.
    ...
    def connect(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    gpib_interface = self.address.split('::')[0]
    ...
    def set_voltage(self, voltage, enable_output=False):
        ...
        if self._voltage_known:
            diff = abs(voltage - self._current_voltage)
            if diff > self._safe_step_size:
    ...
    def ramp_to_voltage(self, target, slew_rate=None, stop_check=None):
        ...
        for i, v in enumerate(voltages):
            ...
            if not self.set_voltage(v, enable_output=False):
...
class MeasurementEngine:
        ...
        try:
            for step_idx, step_val in enumerate(step_values):
                ...
                # Ramp magnet to target while continuously measuring VNA
                    ...
                    # Calculate measurement interval based on sweep time
                    field_range = abs(field_stop - field_start)
                    sweep_time = (field_range / self.field_ramp_rate) * 60  # seconds
                    measurement_interval = max(0.1, sweep_time / max(sweep_points, 10))  # At least 0.1s between measurements
                    
                    print(f"  Expected sweep time: {sweep_time:.1f}s, measurement interval: {measurement_interval:.2f}s")
                    
                    # Set target and start ramping
                    if hasattr(self.magnet, 'set_rate'):
                        self.magnet.set_rate(self.field_ramp_rate)
                    self.magnet.set_field(field_stop)
                    
                    print(f"  Starting continuous measurement...")
                    t_start = time_module.perf_counter()
                    sweep_idx = 0
                    
                    # Continuously measure until we reach target
                    while not self.should_stop:
                        t_loop = time_module.perf_counter()
                        
                        # Read actual field
                        actual_field = self.magnet.get_field()
                        ...
                        # Update progress based on field position
                        if field_stop != field_start:
                            progress_fraction = abs(actual_field - field_start) / abs(field_stop - field_start)
                            progress = (step_idx + progress_fraction) / max(len(step_values), 1) * 100
                            self.progress_queue.put(min(progress, 100))
                        
                        # Check if we've reached target
                        if abs(actual_field - field_stop) <= self.field_tolerance:
                            print(f"  Target field reached: {actual_field:.4f}T")
                            break
                        
                        # Check for timeout (3x expected sweep time)
                        elapsed = time_module.perf_counter() - t_start
                        if elapsed > sweep_time * 3:
                            print(f"  WARNING: Sweep timeout after {elapsed:.1f}s")
                            break
                        
                        # Wait for next measurement interval
                        loop_time = time_module.perf_counter() - t_loop
                        if loop_time < measurement_interval:
                            time.sleep(measurement_interval - loop_time)
                    
                    t_total = time_module.perf_counter() - t_start
                    print(f"  B-field sweep complete: {sweep_idx} points in {t_total:.1f}s")
                    print(f"  Field: {field_start:.4f}T -> {actual_field:.4f}T")
    ...
    def _set_parameter(self, param, value, fixed_values, sweep_config=None, is_step=False):
            ...
            if not self.use_simulation and self.magnet.connected:
                ...
                if is_step and self.wait_for_field:
                    ...
                    while time.time() - start_time < timeout:
                        ...
                        print(f"WARNING: Timeout waiting for field {value:.4f} T")
...
class VNAMeasurementApp:
    ...
    def create_setup_tab(self, parent):
        ...
        self.magnet_addr_entry = ttk.Entry(magnet_addr_frame, textvariable=self.magnet_addr, width=26)
        ...
        # B-Field Settings
        bfield_frame = ttk.LabelFrame(parent, text="B-Field Settings")
        bfield_frame.pack(fill='x', padx=10, pady=5)
        
        # Row 0: Ramp rate and tolerance
        ttk.Label(bfield_frame, text="Ramp Rate (T/min):").grid(row=0, column=0, padx=10, pady=5, sticky='w')
        self.field_ramp_rate = tk.StringVar(value="0.3")
        rate_entry = ttk.Entry(bfield_frame, textvariable=self.field_ramp_rate, width=8)
        rate_entry.grid(row=0, column=1, padx=5, pady=5, sticky='w')
        ToolTip(rate_entry, "B-field ramp rate in Tesla/minute.\nSCM1 max: 0.3 T/min\nValues will be clamped to 0-0.3 range.")
        
        ttk.Label(bfield_frame, text="Tolerance (T):").grid(row=0, column=2, padx=10, pady=5, sticky='w')
        self.field_tolerance = tk.StringVar(value="0.01")
        tol_entry = ttk.Entry(bfield_frame, textvariable=self.field_tolerance, width=8)
        tol_entry.grid(row=0, column=3, padx=5, pady=5, sticky='w')
        ToolTip(tol_entry, "Field tolerance for considering target reached.\n0.01 T = 100 Gauss")
        
        ttk.Label(bfield_frame, text="Settle Time (s):").grid(row=0, column=4, padx=10, pady=5, sticky='w')
        self.field_settle_time = tk.StringVar(value="2.0")
        settle_entry = ttk.Entry(bfield_frame, textvariable=self.field_settle_time, width=8)
        settle_entry.grid(row=0, column=5, padx=5, pady=5, sticky='w')
        ToolTip(settle_entry, "Wait time after field reaches target\nbefore starting measurement.")
        
        # Row 1: Wait for field checkbox and current field display
        self.wait_for_field = tk.BooleanVar(value=True)
        wait_cb = ttk.Checkbutton(
            bfield_frame, text="Wait for field to stabilize (2D sweeps)",
            variable=self.wait_for_field
        )
        wait_cb.grid(row=1, column=0, columnspan=3, padx=10, pady=5, sticky='w')
        ToolTip(wait_cb, "When B-field is the step parameter, wait for\nfield to reach target before starting inner sweep.")
        
        ttk.Label(bfield_frame, text="Current Field:").grid(row=1, column=3, padx=10, pady=5, sticky='w')
        self.current_field_display = ttk.Label(bfield_frame, text="0.000 T", font=('Arial', 10, 'bold'))
        self.current_field_display.grid(row=1, column=4, padx=5, pady=5, sticky='w')
        
        # Row 2: Time estimation
        ttk.Label(bfield_frame, text="Estimated Time:").grid(row=2, column=0, padx=10, pady=5, sticky='w')
        self.time_estimate_display = ttk.Label(bfield_frame, text="--", font=('Arial', 10))
        self.time_estimate_display.grid(row=2, column=1, columnspan=3, padx=5, pady=5, sticky='w')
        
        ttk.Button(bfield_frame, text="Calculate", command=self.calculate_time_estimate).grid(row=2, column=4, padx=5, pady=5)
    ...
    def create_plot_tab(self, parent):
        ...
        for mode in ["Gate Voltage (V)", "Density (cm⁻²)", "Filling Factor (ν)"]:
            ttk.Radiobutton(
        ...
        autoscale_cb = ttk.Checkbutton(xaxis_frame, text="Autoscale", 
                                        variable=self.trace_autoscale_var,
                                        command=self.on_autoscale_changed)
        autoscale_cb.pack(side='right', padx=10)
        
        ttk.Button(xaxis_frame, text="Reset Zoom", command=self.reset_trace_zoom).pack(side='right', padx=5)
        ...
        self.normalize_lines = tk.BooleanVar(value=False)
        norm_cb = ttk.Checkbutton(scale_frame, text="Norm lines", variable=self.normalize_lines, 
                        command=self.update_2d_plot)
        norm_cb.pack(side='left', padx=2)
        ToolTip(norm_cb, "Subtract average from each line to remove offsets")
    ...
    def update_single_trace(self):
        ...
        if xaxis_mode == "Filling Factor (ν)" and sweep_param == "Gate Voltage (V)":
            self._set_filling_factor_ticks(self.ax_trace)
            # Custom cursor format for filling factor
            self.ax_trace.format_coord = lambda x, y: f'ν = {x:.2f}, S21 = {y:.4f} dB'
        elif xaxis_mode == "Density (cm⁻²)" and sweep_param == "Gate Voltage (V)":
            self.ax_trace.format_coord = lambda x, y: f'n = {x:.3e} cm⁻², S21 = {y:.4f} dB'
        elif sweep_param == "Gate Voltage (V)":
        ...
        if self.current_plot_mode == "2D" and self.current_step_index < len(self.sweep_data_2d):
            ...
            if trace_data:
                ...
                if xaxis_mode == "Filling Factor (ν)" and sweep_param == "Gate Voltage (V)":
                    ...
                    self.ax_trace_2d.format_coord = lambda x, y: f'ν = {x:.2f}, S21 = {y:.4f} dB'
                ...
                if hasattr(self, 'sample_density_per_volt') and self.sample_density_per_volt is not None:
                    try:
                        ...
                        if abs(density_per_v) > 1e11:
                            return lambda v: density_per_v * (v - v_cnp) / 1e12, "Density (×10¹² cm⁻²)"
                        else:
                            return lambda v: density_per_v * (v - v_cnp) / 1e10, "Density (×10¹⁰ cm⁻²)"
                    except:
                        pass
                # Fall back to gate voltage
                return lambda v: v, "Gate Voltage (V) [set hBN params]"
            
            elif xaxis_mode == "Filling Factor (ν)":
                ...
                if hasattr(self, 'sample_density_per_volt') and self.sample_density_per_volt is not None:
                    try:
                        ...
                        if b_field != 0:
                            ...
                            h = 6.626e-34
                            e = 1.602e-19
                            # n in m⁻² = density_per_v * 1e4 * (v - v_cnp)
                            # ν = n * h / (e * B)
                            factor = density_per_v * 1e4 * h / (e * b_field)
                            return lambda v, f=factor, vc=v_cnp: f * (v - vc), f"Filling Factor ν (B={b_field:.1f}T)"
                    except:
                        pass
                # Fall back to gate voltage
                return lambda v: v, "Gate Voltage (V) [need B>0]"
        
        # Default: use raw sweep value
        return lambda v: v * freq_scale, sweep_param
    ...
    def update_sample_calculations(self, *args):
        ...
        try:
            ...
            # Capacitance per area: C/A = ε₀ * ε_r / d
            d_m = d_nm * 1e-9  # convert to meters
            cap_per_area = epsilon_0 * epsilon_hbn / d_m  # F/m²
            
            # Density per volt: n = C/A * V / e
            # n [m⁻²] = cap_per_area * 1V / e
            # n [cm⁻²] = n [m⁻²] / 1e4
            density_per_volt = cap_per_area / e / 1e4  # cm⁻² per volt
            
            # Display compact info
            density_str = f"{density_per_volt/1e10:.2f}×10¹⁰ cm⁻²/V"
    ...
    def gate_to_density(self, v_gate):
        """Convert gate voltage to carrier density in cm⁻²."""
        try:
            v_cnp = float(self.v_cnp.get())
    ...
    def gate_to_filling_factor(self, v_gate, b_field):
        ...
        # ν = n * h / (e * B)
        # where n is in m⁻², B in Tesla
        h = 6.626e-34  # J·s
        e = 1.602e-19  # C
        
        # Convert density from cm⁻² to m⁻²
        n_m2 = density * 1e4
            ...
            if step_param and step_param != "None":
                try:
                    ...
                    if step_points > 0:
                        ...
                        self.scan_time_label.config(text=f"2D Est: {time_str} ({step_points} steps × {sweep_str})")
                        return
                except ValueError:
                    pass
    ...
    def start_measurement(self):
        ...
        try:
            ...
            fixed_field = float(self.fixed_field.get())
    ...
    def update_2d_plot(self):
        ...
        # Get x-axis conversion function based on display mode (same as single trace)
        xaxis_mode = self.xaxis_mode.get()
        ...
        # Apply smoothing to each row if enabled
        smooth_window = self.smoothing_window.get()
        ...
        # Normalize each line (subtract row mean) if enabled
        if self.normalize_lines.get():
            row_means = np.nanmean(z, axis=1, keepdims=True)
            z = z - row_means
            zlabel = "Δ" + zlabel  # Indicate normalized data
            cmap = 'RdBu_r'  # Use diverging colormap for normalized data
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            'fixed_temp': self.fixed_temp.get(),
            'ifbw': self.ifbw.get(),
            ...
            'normalize_lines': self.normalize_lines.get(),
        }
    ...
    def load_settings(self):
        ...
        try:
            ...
            if 'temp_port' in settings: self.temp_port.set(settings['temp_port'])
            ...
            if 'fixed_temp' in settings: self.fixed_temp.set(settings['fixed_temp'])
            ...
            if 'normalize_lines' in settings: self.normalize_lines.set(settings['normalize_lines'])
