...
class NHMFLMagnetController:
    ...
    def get_field(self, debug=False):
        ...
        if status:
            ...
            # Sanity check: field should be within magnet range
            if abs(new_field) > self.max_field:
                if debug:
                    print(f"  [SCM1] Rejected bad field reading: {new_field:.4f}T (outside ±{self.max_field}T range)")
                return self.current_field
            ...
            if hasattr(self, 'current_field') and self.current_field is not None:
                max_jump = 0.05  # T - generous margin
                if abs(new_field - self.current_field) > max_jump:
                    if debug:
                        print(f"  [SCM1] Rejected bad field reading: {new_field:.6f}T (jumped {abs(new_field - self.current_field):.4f}T from {self.current_field:.6f}T)")
                    return self.current_field
    ...
    def set_field(self, target_field):
        ...
        try:
            ...
            status = self._get_status_raw()
            if status:
                print(f"NHMFL SCM1: Status: Field={status['Field']:.4f}T, "
                      f"Setpoint={status['Setpoint']:.4f}T, Ramp={status['Ramp']}")
        ...
        try:
            ...
            for step_idx, step_val in enumerate(step_values):
                ...
                # Ramp magnet to target while continuously measuring VNA
                    ...
                    # Make sure magnet is unpaused before we start
                    if hasattr(self.magnet, 'pause'):
                        self.magnet.pause(False)  # Unpause
                    ...
                    t_start = time_module.perf_counter()
                    sweep_idx = 0
                    last_recorded_field = None
                    min_field_step = 0.0001  # Minimum field change to record a new point (0.1 mT)
                    
                    # Continuously measure until we reach target
                    while not self.should_stop:
                        t_loop = time_module.perf_counter()
                        
                        # Read actual field (with debug output)
                        actual_field = self.magnet.get_field(debug=True)
                        
                        # Only record if field has changed significantly
                        if last_recorded_field is not None:
                            if abs(actual_field - last_recorded_field) < min_field_step:
                                # Field hasn't changed enough, skip this point
                        ...
                        last_recorded_field = actual_field
...
class VNAMeasurementApp:
    ...
    def setup_variables(self):
        ...
        self.cpw_slot_width = tk.StringVar(value="10")  # μm
        self.cpw_slot_length = tk.StringVar(value="100")  # μm
    ...
    def create_plot_tab(self, parent):
        ...
        self.trace_display_mode = tk.StringVar(value="Magnitude")
        for mode in ["Magnitude", "Phase", "Real", "Imaginary", "Conductivity"]:
            rb = ttk.Radiobutton(
        ...
        ttk.Label(cpw_frame, text="CPW Geometry (for σ):").pack(side='left', padx=5)
        ...
        ToolTip(slot_w_entry, "CPW slot width in μm\n(gap between center conductor and ground)")
        ttk.Label(cpw_frame, text="μm").pack(side='left')
        ...
        ToolTip(slot_l_entry, "CPW channel length in μm\n(length of graphene over slot)")
        ttk.Label(cpw_frame, text="μm").pack(side='left')
    ...
    def _calculate_conductivity(self, trace_data):
        ...
        Uses formula: σ = -ln(S21/S21_max) × w / (2 × L × Z₀)
        
        Where:
            S21_max is used as reference (σ=0 point)
            w = slot width (μm, converted to m)
            L = slot length (μm, converted to m)
            Z₀ = 50Ω characteristic impedance
        
        Returns conductivity in units of e²/h (quantum conductance units)
        """
        if not trace_data:
            return []
        ...
        # Calculate conductivity: σ = -ln(S21/S21_max) × w / (2 × L × Z₀)
        # Negative sign because S21 < S21_max gives negative ln, but σ should be positive
        # The formula gives sheet conductivity in Siemens
        sigma_S = -np.log(s21_mags / s21_max) * w / (2 * L * Z0)
        
        # Convert to units of e²/h
    ...
    def update_2d_plot(self):
        ...
        if mode == "Magnitude":
            ...
            # Calculate conductivity for each trace
            z_list = []
            for trace in valid_traces:
        ...
        try:
            with open(filepath, 'w') as f:
                ...
                if is_freq_sweep:
                    ...
                    f.write("# Power_at_Probe_dBm, S21_Real, S21_Imag, S21_Mag_dB, S21_Phase_deg\n")
                else:
