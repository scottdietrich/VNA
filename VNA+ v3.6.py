...
class MeasurementEngine:
    ...
    def run_measurement(self, config):
        ...
        if use_vna_sweep:
            ...
            mode_str = 'B-FIELD CONTINUOUS (âš ï¸  interpolated fields, GPIB timeouts cause errors)'
        else:
            if sweep_param == "B-Field (T)":
                if num_averages > 1:
                    mode_str = f'B-FIELD STEPPED ({num_averages}Ã— avg)'
                else:
        ...
        if use_bfield_continuous:
            ...
            print(f"Magnet connected: {self.magnet.connected}, Rate: {self.field_ramp_rate} T/min, Averaging: {num_averages}Ã—")
        print(f"================================")
            ...
            for step_idx, step_val in enumerate(step_values):
                ...
                # Only use this mode when averaging is disabled (num_averages == 1)
                    ...
                    if actual_ramp_rate < requested_rate:
                        if actual_ramp_rate == max_rate_from_vna:
                            print(f"  âš ï¸  Requested rate {requested_rate:.3f} T/min too fast for IFBW={ifbw} Hz")
                            ...
                            print(f"  âš ï¸  Requested rate {requested_rate:.3f} T/min exceeds hardware limit")
                            ...
                            if hasattr(self.magnet, 'query'):
                                ...
                                if response:
                                    ...
                                    if rate_error_percent > 5.0:
                                        print(f"")
                                        print(f"  âš ï¸  WARNING: Hardware rate differs from requested âš ï¸")
                    ...
                    for sweep_idx in range(sweep_points):
                        ...
                        # not before â€” otherwise the anchor says (t=10, field=0.2845)
                        ...
                        if elapsed_check - last_anchor_time >= field_read_interval:
                            ...
                            # Timestamp AFTER the read â€” this is when the field was measured
                    ...
                    # This is the most reliable reading of the sweep â€” no GPIB contention.
                    try:
                    ...
                    if len(sweep_data) >= 2:
                        ...
                        if global_target_fields is None:
                            ...
                            if start_err > 1.0 or stop_err > 1.0:
                                print(f"  âš ï¸  Recorded field range differs from requested:")
                                print(f"      Requested: {field_start:.4f}T -> {field_stop:.4f}T")
                                print(f"      Recorded:  {actual_start:.4f}T -> {actual_stop:.4f}T  (Î”start={start_err:.2f}mT, Î”stop={stop_err:.2f}mT)")
                ...
                # === SLOW PATH: Point-by-point measurement ===
                    ...
                    if sweep_param == "B-Field (T)" and config.get('averages', 1) > 1:
                        num_averages = config.get('averages', 1)
                        print(f"  B-field stepped sweep: {sweep_points} points Ã— {num_averages} averages")
                        ...
                        if sweep_param == "B-Field (T)" and not self.use_simulation and self.magnet.connected:
                            ...
                            for retry in range(max_retries):
                                ...
                                if getattr(self.magnet, '_field_read_fresh', False):
                                    ...
                                    # Stale read - retry
                                    if retry == 0:
                                        print(f"  âš ï¸  Point {sweep_idx+1}: Field read stale, retrying...")
                                    time.sleep(retry_delay)
                            else:
                                # All retries exhausted - this is serious
                                print(f"  âŒ ERROR Point {sweep_idx+1}: All {max_retries} field reads returned stale values!")
                ...
                if sweep_param == "B-Field (T)" and not self.use_simulation:
                    ...
                    if actual_points != expected_points:
                        print(f"  âš ï¸  WARNING: Expected {expected_points} points, got {actual_points}")
                        if self.should_stop:
                            print(f"      Measurement stopped early by user")
                        else:
                            print(f"      This indicates a measurement loop error!")
                    else:
                        print(f"  âœ“ Collected all {actual_points} field points")
                ...
                if should_wait:
                    ...
                    while time.time() - start_time < timeout:
                        ...
                        if not getattr(self.magnet, '_field_read_fresh', False):
                            print(f"  âš ï¸  Field read returned stale cached value, retrying...")
                            time.sleep(0.5)
                            continue
