...
class VNAMeasurementApp:
    ...
    def on_xaxis_mode_changed(self):
        ...
        if self.current_plot_mode == "2D" and len(self.sweep_data_2d) >= 2:
            ...
            if mode == "Filling Factor (ν)" and sweep_param == "Gate Voltage (V)":
                # Revert to previous mode (Density if available, else Voltage)
                self.xaxis_mode.set("Gate Voltage (V)")
                self.status_var.set("⚠️ Filling Factor disabled for 2D maps (use single trace)")
    ...
    def process_data_update(self, data):
        ...
        if data['type'] == 'point':
            ...
            # Update plots - but throttle for performance
            self.update_step_slider()
            
            # Throttle 2D plot updates for large datasets
            # Only update every 5 steps or on last step to prevent GUI lag
            total_steps = self.current_config.get('step_points', 1) if self.current_config else 1
            is_last_step = (step_idx >= total_steps - 1)
            
            # For large measurements (>20 steps), skip frequent plot updates
            if total_steps > 20:
                # Only update plots every 5 steps or at the end
                should_update_plots = is_last_step or (step_idx % 5 == 0)
            else:
    ...
    def update_2d_plot(self, force_rebuild=False):
        ...
        smooth_window = self.smoothing_window.get()
        normalize = self.normalize_lines.get()
        data_hash = (len(valid_traces), n_sweep, mode, smooth_window, normalize)
