...
Version: 3.4 - CNP Normalization (January 2026)
...
class MeasurementEngine:
    ...
    def __init__(self, vna, magnet, keithley, temp_controller, use_simulation=True):
        ...
        self.gate_settle_time = 0.5  # seconds - delay after reaching target voltage
    ...
    def set_gate_safety(self, slew_rate=10.0, ramp_to_zero_after=True, 
        ...
        Args:
            slew_rate: Gate voltage ramp rate in V/s
            ramp_to_zero_after: Ramp to 0V after measurement completes
            ramp_on_stop: Ramp to 0V if measurement is stopped
            max_voltage: Maximum allowed gate voltage
            compliance: Current compliance in Amps
            settle_time: Delay after gate reaches target before measuring (seconds)
        """
        ...
        self.gate_settle_time = settle_time
        ...
        print(f"Gate safety: slew={slew_rate}V/s, max={max_voltage}V, compliance={compliance*1e9:.0f}nA, "
              f"settle={settle_time}s, ramp_to_zero={ramp_to_zero_after}, ramp_on_stop={ramp_on_stop}")
            ...
            for step_idx, step_val in enumerate(step_values):
                ...
                # Set voltage, read it back (forces USB sync), take VNA measurement
                    ...
                    if per_step_reference and step_param == "Frequency (GHz)" and step_val is not None:
                        ...
                        # Settle at reference voltage before measuring
                        if self.gate_settle_time > 0:
                            print(f"    Settling for {self.gate_settle_time}s...")
                            time.sleep(self.gate_settle_time)
                    ...
                    for avg_idx in range(num_averages):
                        ...
                        # Settle at start voltage before beginning sweep
                        if self.gate_settle_time > 0 and avg_idx == 0:
                            print(f"  Settling at start voltage for {self.gate_settle_time}s...")
                            time.sleep(self.gate_settle_time)
                        else:
                            time.sleep(0.05)  # Brief delay for subsequent averages
...
class VNAMeasurementApp:
    ...
    def create_setup_tab(self, parent):
        ...
        # Row 1: Settle time and checkboxes
        ttk.Label(safety_frame, text="Settle (s):").grid(row=1, column=0, padx=5, pady=3, sticky='w')
        self.gate_settle_time = tk.StringVar(value="0.5")
        settle_entry = ttk.Entry(safety_frame, textvariable=self.gate_settle_time, width=6)
        settle_entry.grid(row=1, column=1, padx=2, pady=3, sticky='w')
        ToolTip(settle_entry, "Delay after gate reaches target voltage\nbefore starting measurement (seconds).")
        
        self.ramp_gate_to_zero = tk.BooleanVar(value=True)
        ttk.Checkbutton(safety_frame, text="→0 after", variable=self.ramp_gate_to_zero).grid(
            row=1, column=2, columnspan=2, padx=5, pady=2, sticky='w')
        
        self.ramp_gate_on_stop = tk.BooleanVar(value=False)
        ttk.Checkbutton(safety_frame, text="→0 on Stop", variable=self.ramp_gate_on_stop).grid(
            row=1, column=4, columnspan=2, padx=5, pady=2, sticky='w')
    ...
    def create_sweep_tab(self, parent):
        ...
        self.norm_frame = ttk.LabelFrame(left_frame, text="Gate Voltage Normalization")
        self.norm_frame.pack(fill='x', pady=5)
        
        norm_row1 = ttk.Frame(self.norm_frame)
        norm_row1.pack(fill='x', padx=5, pady=2)
        
        self.norm_checkbox = ttk.Checkbutton(
            norm_row1, text="Enable normalization to reference voltage",
            variable=self.gate_normalization_enabled,
            command=self.on_normalization_changed
        )
        self.norm_checkbox.pack(side='left')
        
        norm_row2 = ttk.Frame(self.norm_frame)
        norm_row2.pack(fill='x', padx=5, pady=2)
        
        ttk.Label(norm_row2, text="Reference V_gate:").pack(side='left', padx=(20, 5))
        self.norm_voltage_entry = ttk.Entry(norm_row2, textvariable=self.gate_normalization_voltage, width=8)
        self.norm_voltage_entry.pack(side='left')
        ttk.Label(norm_row2, text="V").pack(side='left', padx=2)
        
        self.norm_info_label = ttk.Label(
            self.norm_frame, 
            text="Will measure S21 at reference voltage first, then subtract from all measurements",
            font=('Arial', 8), foreground='gray'
        )
        self.norm_info_label.pack(padx=5, pady=2, anchor='w')
    ...
    def update_normalization_visibility(self):
        """Show/hide gate normalization options based on sweep/step parameters.
        
        Normalization is only available when:
        - Gate voltage is the sweep parameter, OR
        - Gate voltage is the step parameter (with frequency as sweep)
        """
        if not hasattr(self, 'norm_frame'):
            return
    ...
    def on_normalization_changed(self):
        ...
        self.norm_voltage_entry.config(state=state)
    
    def is_normalization_applicable(self):
        """Check if normalization is applicable to current sweep/step configuration."""
        sweep = self.sweep_param.get()
        step = self.step_param.get()
    ...
    def start_measurement(self):
        ...
        try:
            ...
            config = {
                ...
                # Gate normalization settings
                'normalization_enabled': self.gate_normalization_enabled.get() and self.is_normalization_applicable(),
        ...
        try:
            ...
            gate_settle = float(self.gate_settle_time.get())
        except ValueError:
            gate_slew = 10.0
            gate_max = 100.0
    ...
    def process_data_update(self, data):
        ...
        if data['type'] == 'reference_data':
            # Store reference data for normalization
            ...
            print(f"[GUI] Reference data received for V={self.reference_voltage}V")
        ...
        try:
            with open(filepath, 'w') as f:
                ...
                try:
                    ...
                    f.write(f"  Settle Time: {self.gate_settle_time.get()} s\n")
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            'gate_settle_time': self.gate_settle_time.get(),
    ...
    def load_settings(self):
        ...
        try:
            ...
            if 'gate_settle_time' in settings: self.gate_settle_time.set(settings['gate_settle_time'])
...
def main():
    """Main entry point."""
    print("=" * 60)
    print("VNA FMR Measurement System v3.4")
    ...
    print("- CNP normalization for gate sweeps")
